...
slitr subsitr(slitr s, vlong start, vlong end) {
	if (start < 0) start += ( vlong ) s.len;
	if (end < 0) end += ( vlong ) s.len;
	if (start < 0) start = 0;
	if (end >= ( vlong ) s.len) end = ( vlong ) s.len - 1;
	if (start > end || s.len == 0) return slitr_empty();
	return mkslitr(s.s + start, ( uvlong ) (end - start + 1));
}

bool slitreq(slitr a, slitr b) {
	if (a.len != b.len) return false;
	if (a.s == b.s) return true;
	if (!a.s || !b.s) return a.len == 0; /* both empty */
	return memcmp(a.s, b.s, ( uvlong ) a.len) == 0;
}

int slitrcmp(slitr a, slitr b) {
	uvlong n = a.len < b.len ? ( uvlong ) a.len : ( uvlong ) b.len;
...
slitr slitrdup(slitr s, int arena) {
	...
	p [s.len] = '\0';
	return mkslitr(p, s.len);
}

/* ═══════════════════════════════════════════════════════
   Strand — dynamic string builder
   Capacity doubles as needed, backed by arena.
   ═══════════════════════════════════════════════════════ */
...
/* Thread-local: strands are created and destroyed within a single thread (e.g.
   fmts/dprint build a strand, observe it, and free it before returning), so the
   descriptor table is per-thread. Lock-free, matching the arena table. */
static _Thread_local strand_t *strands     = null;
static _Thread_local int       nstrands    = 0;
static _Thread_local int       strands_cap = 0;

static bool      strand_table_ensure(int needed) {
	if (needed <= strands_cap) return true;
	uint ucap   = nextpow2(( uint ) needed);
	int  newcap = ( int ) ucap;
	if (newcap < COETUA_STRAND_TABLE_SEED) newcap = COETUA_STRAND_TABLE_SEED;
	strand_t *next = ( strand_t * ) calloc(( size_t ) newcap, sizeof(strand_t));
	if (!next) {
		errmsg("mkstrand: out of memory");
		return false;
	}
	for (int i = nstrands; i < newcap; i++) next [i].arena = -1;
	if (strands) {
		memcpy(next, strands, ( size_t ) nstrands * sizeof(strand_t));
		free(strands);
	}
	strands     = next;
	strands_cap = newcap;
...
static strand_t *strand_get(int sd) {
	if (sd < 0 || sd >= nstrands) return null;
	if (strands [sd].arena < 0) return null;
	return &strands [sd];
}

static bool strand_needed(strand_t *st, uvlong add, uvlong *needed) {
	uvlong len;
	if (!addok64(st->len, add, &len) || !addok64(len, 1, needed)) {
...
static uvlong strand_next_cap(uvlong cap, uvlong needed) {
	uvlong seed = cap ? cap : COETUA_STRAND_CHUNK;
	return needed <= seed ? seed : nextpow2_64(needed);
}

static bool strand_grow(strand_t *st, uvlong needed) {
	if (st->cap >= needed) return true;
	uvlong newcap = strand_next_cap(st->cap, needed);
	char  *nd     = ( char * ) aden(st->arena, newcap);
	if (!nd) return false;
	if (st->data && st->len > 0) memcpy(nd, st->data, ( uvlong ) st->len);
	st->data = nd;
	st->cap  = newcap;
	return true;
}
...
int mkstrand(int arena) {
	/* reuse freed slot (arena == -1 marks freed) */
	for (int i = 0; i < nstrands; i++)
		if (strands [i].arena < 0) {
			strands [i].arena = arena;
			strands [i].data  = null;
			strands [i].cap   = 0;
			strands [i].len   = 0;
			return i;
		}
	if (!strand_table_ensure(nstrands + 1)) return -1;
	int id             = nstrands++;
	strands [id].arena = arena;
	strands [id].data  = null;
	strands [id].cap   = 0;
	strands [id].len   = 0;
	return id;
}
...
int catstr(int arena, ...) {
	...
	if (st < 0) return -1;
	va_start(ap, arena);
	while ((s = va_arg(ap, char *)) != null) concat(st, s);
	va_end(ap);
	return st;
}
...
void concat(int strand, char *s) {
	if (!s) return;
	concats(strand, mkslitr(( char * ) s, strlen(s)));
}
...
void concats(int strand, slitr s) {
	...
	memcpy(st->data + st->len, s.s, ( uvlong ) s.len);
	st->len            += s.len;
	st->data [st->len]  = '\0';
}

void concatr(int strand, rune r) {
	char buf [4];
	int  n = runetochar(buf, &r);
	if (n <= 0) return;
	concats(strand, mkslitr(buf, ( uvlong ) n));
}
...
void putstr(int strand, vlong pos, char *s) {
	...
	uvlong needed;
	if (!strand_needed(st, slen, &needed)) return;
	if (!strand_grow(st, needed)) return;
	uvlong at = ( uvlong ) pos;
	memmove(st->data + at + slen, st->data + at, ( uvlong ) (st->len - at));
	memcpy(st->data + at, s, slen);
	st->len            += ( uvlong ) slen;
	st->data [st->len]  = '\0';
}
...
void dropstr(int strand, vlong pos, vlong n) {
	...
	if (pos < 0 || ( uvlong ) pos >= st->len) return;
	uvlong at = ( uvlong ) pos;
	if (( uvlong ) n > st->len - at) n = ( vlong ) (st->len - at);
	memmove(st->data + at, st->data + at + n, ( uvlong ) (st->len - at - ( uvlong ) n));
	st->len            -= ( uvlong ) n;
	st->data [st->len]  = '\0';
}
...
void repeat(int strand, uvlong n) {
	...
	if (!strand_grow(st, needed)) return;
	uvlong orig = st->len;
	for (uvlong i = 1; i < n; i++) memcpy(st->data + orig * i, st->data, ( uvlong ) orig);
	st->len            = len;
	st->data [st->len] = '\0';
}
...
uvlong tknize(char *str, char **arr, uvlong max) {
	...
	for (;;) {
		while (*r && isspacerune(( uchar ) *r)) r++;
		if (!*r) break;

		if (n < max && arr) arr [n] = w;
		n++;

		while (*r && !isspacerune(( uchar ) *r)) {
			if (*r != '\'') {
				*w++ = *r++;
				continue;
			}
...
int chartorune(rune *rp, char *s) {
	...
	if (c < T4) {
		...
		if (l <= Rune2 || (l >= 0xd800 && l <= 0xdfff)) goto bad;
		*rp = ( rune ) l;
		return 3;
	}
...
int runetochar(char *s, rune *rp) {
	long c = *rp;
	if (c < 0 || (c >= 0xd800 && c <= 0xdfff) || c > Runemax) c = Runeerror;
	if (c <= Rune1) {
		s [0] = ( char ) c;
		return 1;
	}
	if (c <= Rune2) {
		s [0] = ( char ) (T2 | (c >> 1 * Bitx));
		s [1] = ( char ) (Tx | (c & Maskx));
		return 2;
	}
	if (c <= Rune3) {
		s [0] = ( char ) (T3 | (c >> 2 * Bitx));
		s [1] = ( char ) (Tx | ((c >> 1 * Bitx) & Maskx));
		s [2] = ( char ) (Tx | (c & Maskx));
		return 3;
	}
	s [0] = ( char ) (T4 | (c >> 3 * Bitx));
	s [1] = ( char ) (Tx | ((c >> 2 * Bitx) & Maskx));
	s [2] = ( char ) (Tx | ((c >> 1 * Bitx) & Maskx));
	s [3] = ( char ) (Tx | (c & Maskx));
	return 4;
}
...
char *utfrune(char *s, long c) {
	if (c < Runesync) return strchr(s, ( int ) c);
	for (;;) {
		int c1 = *( uchar * ) s;
		if (c1 < Runeself) {
			if (c1 == 0) return null;
		...
		rune r;
		int  n = chartorune(&r, s);
		if (r == ( rune ) c) return s;
		s += n;
	}
}

char *utfrrune(char *s, long c) {
	if (c < Runesync) return strrchr(s, ( int ) c);
	char *last = null;
	for (;;) {
		int c1 = *( uchar * ) s;
		if (c1 < Runeself) {
...
char *utfutf(char *s1, char *s2) { return strstr(s1, s2); }

char *utfecpy(char *to, char *eto, char *from) {
	if (to >= eto) return to;
	char *p = to;
	while (p < eto)
		if ((*p++ = *from++) == 0) return p - 1;
	p = eto;
	while (p > to && ((*--p & 0xc0) == 0x80));
	*p = 0;
	return p;
}

/* Plan 9 rune type tables and binary-search case logic. */



static rune __alpha2 [] = {
  0x00d8, 0x00f6, 0x00f8, 0x01f5, 0x0250, 0x02a8, 0x038e, 0x03a1, 0x03a3, 0x03ce, 0x03d0, 0x03d6, 0x03e2, 0x03f3,
  0x0490, 0x04c4, 0x0561, 0x0587, 0x05d0, 0x05ea, 0x05f0, 0x05f2, 0x0621, 0x063a, 0x0640, 0x064a, 0x0671, 0x06b7,
  0x06ba, 0x06be, 0x06c0, 0x06ce, 0x06d0, 0x06d3, 0x0905, 0x0939, 0x0958, 0x0961, 0x0985, 0x098c, 0x098f, 0x0990,
  0x0993, 0x09a8, 0x09aa, 0x09b0, 0x09b6, 0x09b9, 0x09dc, 0x09dd, 0x09df, 0x09e1, 0x09f0, 0x09f1, 0x0a05, 0x0a0a,
  0x0a0f, 0x0a10, 0x0a13, 0x0a28, 0x0a2a, 0x0a30, 0x0a32, 0x0a33, 0x0a35, 0x0a36, 0x0a38, 0x0a39, 0x0a59, 0x0a5c,
  0x0a85, 0x0a8b, 0x0a8f, 0x0a91, 0x0a93, 0x0aa8, 0x0aaa, 0x0ab0, 0x0ab2, 0x0ab3, 0x0ab5, 0x0ab9, 0x0b05, 0x0b0c,
  0x0b0f, 0x0b10, 0x0b13, 0x0b28, 0x0b2a, 0x0b30, 0x0b32, 0x0b33, 0x0b36, 0x0b39, 0x0b5c, 0x0b5d, 0x0b5f, 0x0b61,
  0x0b85, 0x0b8a, 0x0b8e, 0x0b90, 0x0b92, 0x0b95, 0x0b99, 0x0b9a, 0x0b9e, 0x0b9f, 0x0ba3, 0x0ba4, 0x0ba8, 0x0baa,
  0x0bae, 0x0bb5, 0x0bb7, 0x0bb9, 0x0c05, 0x0c0c, 0x0c0e, 0x0c10, 0x0c12, 0x0c28, 0x0c2a, 0x0c33, 0x0c35, 0x0c39,
  0x0c60, 0x0c61, 0x0c85, 0x0c8c, 0x0c8e, 0x0c90, 0x0c92, 0x0ca8, 0x0caa, 0x0cb3, 0x0cb5, 0x0cb9, 0x0ce0, 0x0ce1,
  0x0d05, 0x0d0c, 0x0d0e, 0x0d10, 0x0d12, 0x0d28, 0x0d2a, 0x0d39, 0x0d60, 0x0d61, 0x0e01, 0x0e30, 0x0e32, 0x0e33,
  0x0e40, 0x0e46, 0x0e5a, 0x0e5b, 0x0e81, 0x0e82, 0x0e87, 0x0e88, 0x0e94, 0x0e97, 0x0e99, 0x0e9f, 0x0ea1, 0x0ea3,
  0x0eaa, 0x0eab, 0x0ead, 0x0eae, 0x0eb2, 0x0eb3, 0x0ec0, 0x0ec4, 0x0edc, 0x0edd, 0x0f18, 0x0f19, 0x0f40, 0x0f47,
  0x0f49, 0x0f69, 0x10d0, 0x10f6, 0x1100, 0x1159, 0x115f, 0x11a2, 0x11a8, 0x11f9, 0x1e00, 0x1e9b, 0x1f50, 0x1f57,
  0x1f80, 0x1fb4, 0x1fb6, 0x1fbc, 0x1fc2, 0x1fc4, 0x1fc6, 0x1fcc, 0x1fd0, 0x1fd3, 0x1fd6, 0x1fdb, 0x1fe0, 0x1fec,
  0x1ff2, 0x1ff4, 0x1ff6, 0x1ffc, 0x210a, 0x2113, 0x2115, 0x211d, 0x2120, 0x2122, 0x212a, 0x2131, 0x2133, 0x2138,
  0x3041, 0x3094, 0x30a1, 0x30fa, 0x3105, 0x312c, 0x3131, 0x318e, 0x3192, 0x319f, 0x3260, 0x327b, 0x328a, 0x32b0,
  0x32d0, 0x32fe, 0x3300, 0x3357, 0x3371, 0x3376, 0x337b, 0x3394, 0x3399, 0x339e, 0x33a9, 0x33ad, 0x33b0, 0x33c1,
  0x33c3, 0x33c5, 0x33c7, 0x33d7, 0x33d9, 0x33dd, 0x4e00, 0x9fff, 0xac00, 0xd7a3, 0xf900, 0xfb06, 0xfb13, 0xfb17,
  0xfb1f, 0xfb28, 0xfb2a, 0xfb36, 0xfb38, 0xfb3c, 0xfb40, 0xfb41, 0xfb43, 0xfb44, 0xfb46, 0xfbb1, 0xfbd3, 0xfd3d,
  0xfd50, 0xfd8f, 0xfd92, 0xfdc7, 0xfdf0, 0xfdf9, 0xfe70, 0xfe72, 0xfe76, 0xfefc, 0xff66, 0xff6f, 0xff71, 0xff9d,
  0xffa0, 0xffbe, 0xffc2, 0xffc7, 0xffca, 0xffcf, 0xffd2, 0xffd7, 0xffda, 0xffdc,
};


static rune __alpha1 [] = {
  0x00aa, 0x00b5, 0x00ba, 0x03da, 0x03dc, 0x03de, 0x03e0, 0x06d5, 0x09b2, 0x0a5e, 0x0a8d,
  0x0ae0, 0x0b9c, 0x0cde, 0x0e4f, 0x0e84, 0x0e8a, 0x0e8d, 0x0ea5, 0x0ea7, 0x0eb0, 0x0ebd,
  0x1fbe, 0x207f, 0x20a8, 0x2102, 0x2107, 0x2124, 0x2126, 0x2128, 0xfb3e, 0xfe74,
};


static rune __space2 [] = {
  0x0009, 0x000a, 0x0020, 0x0020, 0x00a0, 0x00a0, 0x2000, 0x200b, 0x2028, 0x2029, 0x3000, 0x3000, 0xfeff, 0xfeff,
};


static rune __toupper2 [] = {
  0x0061, 0x007a, 468, 0x00e0, 0x00f6, 468, 0x00f8, 0x00fe, 468, 0x0256, 0x0257, 295, 0x0258, 0x0259, 298,
  0x028a, 0x028b, 283, 0x03ad, 0x03af, 463, 0x03b1, 0x03c1, 468, 0x03c3, 0x03cb, 468, 0x03cd, 0x03ce, 437,
  0x0430, 0x044f, 468, 0x0451, 0x045c, 420, 0x045e, 0x045f, 420, 0x0561, 0x0586, 452, 0x1f00, 0x1f07, 508,
  0x1f10, 0x1f15, 508, 0x1f20, 0x1f27, 508, 0x1f30, 0x1f37, 508, 0x1f40, 0x1f45, 508, 0x1f60, 0x1f67, 508,
  0x1f70, 0x1f71, 574, 0x1f72, 0x1f75, 586, 0x1f76, 0x1f77, 600, 0x1f78, 0x1f79, 628, 0x1f7a, 0x1f7b, 612,
  0x1f7c, 0x1f7d, 626, 0x1f80, 0x1f87, 508, 0x1f90, 0x1f97, 508, 0x1fa0, 0x1fa7, 508, 0x1fb0, 0x1fb1, 508,
  0x1fd0, 0x1fd1, 508, 0x1fe0, 0x1fe1, 508, 0x2170, 0x217f, 484, 0x24d0, 0x24e9, 474, 0xff41, 0xff5a, 468,
};
...
static rune __tolower2 [] = {
  0x0041, 0x005a, 532,    0x00c0, 0x00d6, 532,    0x00d8, 0x00de, 532,    0x0189, 0x018a, 705,    0x018e, 0x018f,
  702,    0x01b1, 0x01b2, 717,    0x0388, 0x038a, 537,    0x038e, 0x038f, 563,    0x0391, 0x03a1, 532,    0x03a3,
  0x03ab, 532,    0x0401, 0x040c, 580,    0x040e, 0x040f, 580,    0x0410, 0x042f, 532,    0x0531, 0x0556, 548,
  0x10a0, 0x10c5, 548,    0x1f08, 0x1f0f, 492,    0x1f18, 0x1f1d, 492,    0x1f28, 0x1f2f, 492,    0x1f38, 0x1f3f,
  492,    0x1f48, 0x1f4d, 492,    0x1f68, 0x1f6f, 492,    0x1f88, 0x1f8f, 492,    0x1f98, 0x1f9f, 492,    0x1fa8,
  0x1faf, 492,    0x1fb8, 0x1fb9, 492,    0x1fba, 0x1fbb, 426,    0x1fc8, 0x1fcb, 414,    0x1fd8, 0x1fd9, 492,
  0x1fda, 0x1fdb, 400,    0x1fe8, 0x1fe9, 492,    0x1fea, 0x1feb, 388,    0x1ff8, 0x1ff9, 372,    0x1ffa, 0x1ffb,
  374,    0x2160, 0x216f, 516,    0x24b6, 0x24cf, 526,    0xff21, 0xff3a, 532,
};
...
static rune __totitle1 [] = {
  0x01c4, 501, 0x01c6, 499, 0x01c7, 501, 0x01c9, 499, 0x01ca, 501, 0x01cc, 499, 0x01f1, 501, 0x01f3, 499,
};

static rune *rtsearch(rune c, rune *t, int n, int ne) {
	rune *p;
	int   m;
...
rune tolowerrune(rune c) {
	...
	p = rtsearch(c, __tolower2, (sizeof(__tolower2) / sizeof((__tolower2) [0])) / 3, 3);
	if (p && c >= p [0] && c <= p [1]) return c + p [2] - 500;
	p = rtsearch(c, __tolower1, (sizeof(__tolower1) / sizeof((__tolower1) [0])) / 2, 2);
...
rune toupperrune(rune c) {
	...
	p = rtsearch(c, __toupper2, (sizeof(__toupper2) / sizeof((__toupper2) [0])) / 3, 3);
	if (p && c >= p [0] && c <= p [1]) return c + p [2] - 500;
	p = rtsearch(c, __toupper1, (sizeof(__toupper1) / sizeof((__toupper1) [0])) / 2, 2);
	if (p && c == p [0]) return c + p [1] - 500;
	return c;
}
...
rune totitlerune(rune c) {
	...
	p = rtsearch(c, __totitle1, (sizeof(__totitle1) / sizeof((__totitle1) [0])) / 2, 2);
	if (p && c == p [0]) return c + p [1] - 500;
	return c;
}
...
int isalpharune(rune c) {
	...
	p = rtsearch(c, __alpha2, (sizeof(__alpha2) / sizeof((__alpha2) [0])) / 2, 2);
	if (p && c >= p [0] && c <= p [1]) return 1;
	p = rtsearch(c, __alpha1, sizeof(__alpha1) / sizeof((__alpha1) [0]), 1);
	if (p && c == p [0]) return 1;
	return 0;
}

int istitlerune(rune c) { return c == totitlerune(c) && isupperrune(c); }

int isspacerune(rune c) {
	rune *p;

	p = rtsearch(c, __space2, (sizeof(__space2) / sizeof((__space2) [0])) / 2, 2);
	if (p && c >= p [0] && c <= p [1]) return 1;
	return 0;
}
