...
/* Print a computed PIID in canonical form. */
...
/* Compute PIIDs for a curated set of generic instantiations for the system
   differential check. */
...
/* IReference`1<Guid> — System.Guid is not a Windows TypeDef, so its arg sig is the
   basic "g16"; verify against the system implementation directly. */
...
static int output_replace(char const *tmp, char const *path) {
#ifdef _WIN32
...
static void output_discard(int bh, int bc, char const *htmp, char const *ctmp) {
	if (bh >= 0) rmbio(bh);
	if (bc >= 0) rmbio(bc);
	remove(htmp);
	remove(ctmp);
...
static int piid_write_header(int bh, piid_set const *s) {
	uint32_t i;
	if (bprint(bh, "/* Generated by cwinrt-gen; do not edit. Parameterized interface IDs\n"
	...
	for (i = 0; i < s->n; i++) {
		...
		if (bprint(bh, "extern const IID %s;\n", sym) < 0) return -1;
	}
	return bprint(bh, "\n#endif\n") < 0 ? -1 : 0;
}

static int piid_write_impl(int bc, piid_set const *s) {
	uint32_t i;
	if (bprint(bc, "/* Generated by cwinrt-gen; do not edit. PIID definitions. */\n"
	...
	for (i = 0; i < s->n; i++) {
		...
		char           sym [1100];
		cwinrt_name_iid_symbol(s->names [i], sym, sizeof(sym));
		if (bprint(bc, "const IID %s = { 0x%02X%02X%02X%02X, 0x%02X%02X, 0x%02X%02X, "
		               "{ 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X } };\n",
		           sym, g [3], g [2], g [1], g [0], g [5], g [4], g [7], g [6], g [8], g [9], g [10], g [11],
		           g [12], g [13], g [14], g [15])
		    < 0)
			return -1;
	}
...
/* Write the PIID header + definition unit. Returns 0, or -1 on output failure. */
static int piid_set_write(piid_set const *s, char const *out_dir, char const *impl_dir) {
	char     hpath [1024];
	char     cpath [1024];
	char     htmp [1032];
	char     ctmp [1032];
	...
	snprintf(cpath, sizeof(cpath), "%s/cwinrt_piids.impl.c", impl_dir);
	snprintf(htmp, sizeof(htmp), "%s.tmp", hpath);
	...
	bh = bopen(arena, htmp, mod);
	bc = bopen(arena, ctmp, mod);
	if (bh < 0 || bc < 0) {
		output_discard(bh, bc, htmp, ctmp);
		rmarena(arena);
		errmsg("emit-piids: cannot open output");
		return -1;
	}
	rc = piid_write_header(bh, s) | piid_write_impl(bc, s);
	if (rmbio(bh) != 0) rc = -1;
	if (rmbio(bc) != 0) rc = -1;
	rmarena(arena);
	if (rc != 0) {
		remove(htmp);
		remove(ctmp);
		errmsg("emit-piids: cannot write output");
		return -1;
	}
	if (output_replace(ctmp, cpath) != 0 || output_replace(htmp, hpath) != 0) {
		remove(htmp);
		remove(ctmp);
		errmsg("emit-piids: cannot replace output");
		return -1;
	}
...
int gen_emit_piids(char const *winmd_path, char const *out_dir, char const *impl_dir) {
	winmd_db wm;
	piid_set s;
	int      rc;

	if (!out_dir) out_dir = "include/cwinrt";
	if (!impl_dir) impl_dir = "include/cwinrt/impl";
	if (winmd_open(winmd_path, &wm) != 0) return -1;
	memset(&s, 0, sizeof(s));
	piid_set_collect(&wm, &s);
	rc = piid_set_write(&s, out_dir, impl_dir);
	piid_set_free(&s);
	winmd_close(&wm);
	return rc;
}
